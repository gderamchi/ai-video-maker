const { storeJob, updateJob } = require('./job-storage');
...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        console.log('Starting background video generation job:', jobId);
        ...
        // THIS IS THE KEY: Use context.callbackWaitsForEmptyEventLoop = false
        // This allows the function to return immediately while keeping the event loop alive
        context.callbackWaitsForEmptyEventLoop = false;

        // Start async video generation in the background
        generateVideoAsync(jobId, prompt, photos, apiKey);
        ...
        console.error('Error in start-video-background function:', error);
        return {
            statusCode: 500,
            body: JSON.stringify({ 
